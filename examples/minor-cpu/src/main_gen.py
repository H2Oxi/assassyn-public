...
def Valid_Checker(
    ...
    a_valid =(exec_bypass_reg[0] == rs1) | (mem_bypass_reg[0] == rs1) | ~rs1_valid | (~(on_write >> rs1))[0:0] #| (wb_bypass_reg[0] == rs1)
    a_valid_true = a_valid.select(Bits(1)(1),(wb_bypass_reg[0] == rs1).bitcast(Bits(1))) 
    with Condition(~a_valid):
        log("exec_bypass_reg: x{:02} | mem_bypass_reg: x{:02} | ~signals.rs1_valid: {} | (~(on_write >> rs1))[0:0]: {} |", exec_bypass_reg[0], mem_bypass_reg[0], ~rs1_valid, (~(on_write >> rs1))[0:0])
    b_valid =(exec_bypass_reg[0] == rs2) | (mem_bypass_reg[0] == rs2) | ~rs2_valid | (~(on_write >> rs2))[0:0] #| (wb_bypass_reg[0] == rs2)
    b_valid_true = b_valid.select(Bits(1)(1),(wb_bypass_reg[0] == rs2).bitcast(Bits(1) ))
    with Condition(~b_valid):
        log("exec_bypass_reg: x{:02} | mem_bypass_reg: x{:02} | ~signals.rs2_valid: {} | (~(on_write >> rs2))[0:0]: {} |", exec_bypass_reg[0], mem_bypass_reg[0], ~rs2_valid, (~(on_write >> rs2))[0:0])
    rd_valid =  (exec_bypass_reg[0] == rd) | (mem_bypass_reg[0] == rd) | ~rd_valid | (~(on_write >> rd))[0:0] 
    rd_valid_true = rd_valid.select(Bits(1)(1),(wb_bypass_reg[0] == rd).bitcast(Bits(1) ))
    with Condition(~rd_valid):
        log("exec_bypass_reg: x{:02} | mem_bypass_reg: x{:02} | ~signals.rd_valid: {} | (~(on_write >> rd))[0:0]: {} |", exec_bypass_reg[0], mem_bypass_reg[0], ~rd_valid, (~(on_write >> rd))[0:0])
    valid = a_valid_true & b_valid_true & rd_valid_true
    with Condition(~valid):
        log("pc: 0x{:08x}   | rs1-x{:02}: {}       | rs2-x{:02}: {}   | rd-x{:02}: {} | backlogged", \
            fetch_addr, rs1, a_valid, rs2, b_valid, rd, rd_valid)
        
    return valid , a_valid , b_valid

'''
@generated
class Operand_Fetcher(External):
...
class Execution(Module):
    ...
    def build(
        ...
        valid , a_valid , b_valid = Valid_Checker(
            rs1=rs1,
            rs1_valid=signals.rs1_valid,
            rs2=rs2,
            rs2_valid=signals.rs2_valid,
            rd=signals.rd,
            rd_valid=signals.rd_valid,
